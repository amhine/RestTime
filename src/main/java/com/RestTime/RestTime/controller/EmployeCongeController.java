...
public class EmployeCongeController {
    ...
    public ResponseEntity<DemandeConge> soumettre(@PathVariable Long userId, @RequestBody DemandeCongeCreateDTO dto) {
        return ResponseEntity.ok(congeService.soumettreDemande(userId, dto));
    }

    @GetMapping("/{userId}/mesdemandes")
    public ResponseEntity<List<DemandeConge>> getMesDemandes(@PathVariable Long userId) {
